...
// increments all the nested connection ids
const createSeed = (i:number):string => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const incrementId = (seed:string, increment:number, idReg:any, id:string): string => {
    // get all connects
    const connections = seed.match(idReg)
  
    // get all ids from the connect
    const idsTransformed = connections.map((x) => {
      return `${id}: ${Number(
        x.match(/[0-9]+/g)[0]
      ) + increment}`
    })
    ...
    return seed.replace(idReg, getSynonym)
