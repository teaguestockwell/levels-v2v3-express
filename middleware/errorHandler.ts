...
const handle = (body, req: Request, res: Response) => {
  try{
    if(!body){
      if(res.statusCode === 200){return {msg: `200: ${req.method} @ ${req.originalUrl}`}}
      if(res.statusCode  === 400){return {msg: `400: Name must be unique to ${req.method} @ ${req.originalUrl}`}}
      if(res.statusCode === 403){return {msg: `403: You do not have a high enough role to ${req.method} @ ${req.originalUrl}`}}
      return {
